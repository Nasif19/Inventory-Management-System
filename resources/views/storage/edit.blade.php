...
<div class="main-card mb-3 card">
    <div class="card-header">Update Storage</div>    
    <div class="card-body">
        ...
        @endif 
        <form method="POST" action="{{route('store.update',$data->id)}}">
            @csrf
            @method('PUT')
            ...
            <div class="position-relative form-group">
                ...
                <input name="address" value="{{$data->address}}" id="address" placeholder="Address here.." type="" class="form-control @error('address') is-invalid @enderror">
                @error('address')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                    </span>
            ...
            <center> <button class="mb-2 mr-2 btn btn-lg btn-hover-shine btn-primary" type="submit">{{_('Update')}}</button></center>
        </form>
    </div>
</div>
